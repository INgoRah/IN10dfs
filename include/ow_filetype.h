...
struct bitfield {
	char * link ; // name of property
	unsigned int size ; // number of bits
	unsigned int shift ; // bits to shift over
} ;

int FS_r_bitfield(struct one_wire_query *owq) ;
int FS_w_bitfield(struct one_wire_query *owq) ;
int FS_r_bit_array(struct one_wire_query *owq) ;
int FS_w_bit_array(struct one_wire_query *owq) ;
