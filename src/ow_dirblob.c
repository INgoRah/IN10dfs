...
int DirblobAdd(const uint8_t * sn, struct dirblob *db)
{
	if ( db->troubled ) {
		return -EINVAL ;
	}
	...
	if ((db->devices >= db->allocated) || (db->snlist == NULL)) {
		...
		uint8_t *try_bigger_block = realloc(db->snlist, DIRBLOB_ELEMENT_LENGTH * newalloc);
...
int DirblobGet(int device_index, uint8_t * sn, const struct dirblob *db)
{
 ...
 */
int DirblobSearch(uint8_t * sn, const struct dirblob *db)
{
	int device_index;
...
int DirblobRecreate( uint8_t * snlist, int size, struct dirblob *db)
{
	DirblobInit(db) ;
