...
static int FS_structdevdir(void (*dirfunc) (void *, const struct parsedname *), void *v, const struct parsedname *pn_device_directory);
...
static int FS_cache_or_real(void (*dirfunc) (void *, const struct parsedname * const), void *v, const struct parsedname *pn2, uint32_t * flags);
...
static int FS_dir_both(void (*dirfunc) (void *, const struct parsedname *), void *v, const struct parsedname *pn_raw_directory, uint32_t * flags)
{
	...
	if (pn_raw_directory->selected_filetype != NO_FILETYPE) {
		...
		LEVEL_DEBUG("type=%d", pn_raw_directory->type);
		...
		if (ShouldReturnBusList(pn_raw_directory)) {
			LEVEL_DEBUG("ret=%d", ret);
			// only add funny directories for branches
...
static void FS_dir_all_connections_callback_conn( struct dir_all_connections_struct * dacs )
{
	...
	if (TestConnection(&(dacs->pn_directory)) != 0) {	// reconnect ok?
		...
		printf("FS_dir_all_connections: Call FS_cache_or_real %s\n", dacs->pn_directory.path);
...
static void *FS_dir_all_connections_callback_port(void *v)
{
	...
	if ( dacs_next.pin == NULL ) {
		...
		threadbad = pthread_create(&thread, NULL, FS_dir_all_connections_callback_port, (void *) (&dacs_next));
	}
...
static int FS_dir_all_connections(void (*dirfunc) (void *, const struct parsedname *), void *v, const struct parsedname *pn_directory, uint32_t * flags)
{
	if (BAD(TestConnection(pn_directory))) {	// reconnect ok?
		return -ECONNABORTED;
	} else if (IsAlarmDir(pn_directory)) {	/* root or branch directory -- alarm state */
		//printf("FS_dir_all_connections: Call FS_alarmdir %s\n", dacs->pn_directory->path);
		return FS_alarmdir(dirfunc, v, pn_directory);
	} else {
		printf("FS_dir_all_connections: Call FS_cache_or_real %s\n", pn_directory->path);
		return FS_cache_or_real(dirfunc, v, pn_directory, flags);
	}
...
static int FS_realdir(void (*dirfunc) (void *, const struct parsedname *), void *v, const struct parsedname *pn_whole_directory, uint32_t * flags)
{
	...
	LEVEL_DEBUG("seach? pn->fd=%d", pn_whole_directory->selected_connection->pown->file_descriptor);
...
static int FS_cache_or_real(void (*dirfunc) (void *, const struct parsedname *), void *v, const struct parsedname *pn_real_directory, uint32_t * flags)
{
	size_t dindex;
	struct dirblob db;
...
static int FS_busdir(void (*dirfunc) (void *, const struct parsedname *), void *v, const struct parsedname *pn_directory)
{
	...
	for ( int i = 0; i != 4; i++ ) {
		snprintf(bus, OW_FULLNAME_MAX, "bus.%d", i);
