...
#define TREE_DATA(tn)    ( (uint8_t *)(tn) + sizeof(struct tree_node) )
#define CONST_TREE_DATA(tn)    ( (const uint8_t *)(tn) + sizeof(struct tree_node) )

#define ALIAS_TREE_DATA(atn)    ( (char *)(atn) + sizeof(struct alias_tree_node) )
#define CONST_ALIAS_TREE_DATA(atn)    ( (const char *)(atn) + sizeof(struct alias_tree_node) )
...
static int Cache_Get_Alias_Common( struct alias_tree_node * atn) ;

static void Cache_Add_Alias_SN(const char * alias_name, const uint8_t * sn) ;
static void Cache_Del_Alias_SN(const char * alias_name) ;

static void Cache_Add_Alias_Persistent(struct alias_tree_node *atn);
static GOOD_OR_BAD Cache_Get_Alias_Persistent( uint8_t * sn, struct alias_tree_node * atn);
...
static void LoadTK( const uint8_t * sn, void * p, int extension, struct tree_node * tn ) ;
...
static int IsThisPersistent( const struct parsedname * pn )
{
	return ((pn->selected_filetype->change == fc_persistent) || 0) ;
}
...
{
	Cache_Clear() ;
	if (cache.persistent_tree)
		free(cache.persistent_tree);
	if (cache.persistent_alias_tree)
		free(cache.persistent_alias_tree);
}
...
/* return 0 if good, 1 if not */
GOOD_OR_BAD Cache_Add_Device(const int bus_nr, const uint8_t * sn)
{
...
GOOD_OR_BAD Cache_Add_SlaveSpecific(const void *data, const size_t datasize, const struct internal_prop *ip, const struct parsedname *pn)
{
	...
	//printf("  ADD INTERNAL data[0]=%d size=%d \n",((uint8_t *)data)[0],datasize);
	switch (ip->change) {
	case fc_persistent:
		return Cache_Add_Persistent(tn);
	default:
		return Cache_Add_Common(tn);
	}
}
...
/* Add an item to the cache */
/* return 0 if good, 1 if not */
GOOD_OR_BAD Cache_Add_Alias(const char *name, const uint8_t * sn)
{
	struct tree_node *tn;
	...
	tn->expires = NOW_TIME;
	tn->dsize = size;
	memcpy((char *)TREE_DATA(tn), name, size+1 ); // includes NULL
...
static GOOD_OR_BAD Cache_Add_Common(struct tree_node *tn)
{
	...
	if ((opaque = tsearch(tn, &cache.temporary_tree_new, tree_compare))) {
...
GOOD_OR_BAD Cache_Get(void *data, size_t * dsize, const struct parsedname *pn)
{
	...
	LoadTK( pn->sn, pn->selected_filetype, pn->extension, &tn );
	return persistent ?
		Cache_Get_Persistent(data, dsize, &duration, &tn) :
...
GOOD_OR_BAD Cache_Get_Dir(struct dirblob *db, const struct parsedname *pn)
{
	...
	LoadTK( pn_directory.sn, Directory_Marker, 0, &tn) ;
	return Cache_Get_Common_Dir(db, &duration, &tn);
}
...
GOOD_OR_BAD Cache_Get_Device(void *bus_nr, const struct parsedname *pn)
{
	...
	return Cache_Get_Common(bus_nr, &size, &duration, &tn);
}
...
static GOOD_OR_BAD Cache_Get_Internal(void *data, size_t * dsize, const struct internal_prop *ip, const struct parsedname *pn)
{
	...
	switch (ip->change) {
		case fc_persistent:
			return Cache_Get_Persistent(data, dsize, &duration, &tn);
		default:
...
GOOD_OR_BAD Cache_Get_Simul_Time(const struct internal_prop *ip, time_t * dwell_time, const struct parsedname * pn)
{
	...
	if (Cache_Get_Common(NULL, &dsize_simul, &duration, &tn)) {
		return gbBAD ;
	}
...
char * Cache_Get_Alias(const uint8_t * sn)
{
	struct tree_node tn;
	struct tree_opaque *opaque;
	...
	if ( opaque != NULL ) {
		...
		if ( alias_name != NULL ) {
			memcpy( alias_name, (char *)TREE_DATA(opaque->key), opaque->key->dsize+1 ) ;
...
void Cache_Del_Alias(const uint8_t * sn)
{
	char * alias_name ;
	struct tree_node *tn;
	size_t size ;
	...
	if ( tn != NULL ) {
		tn->expires = NOW_TIME;
		tn->dsize = size;
		memcpy((char *)TREE_DATA(tn), alias_name, size+1); // includes NULL
		LoadTK( sn, Alias_Marker, 0, tn ) ;
		Cache_Del_Persistent(tn);
...
static void Aliaslistaction(const void *node, const VISIT which, const int depth)
{
	...
	case postorder:
		...
		MemblobAdd( (uint8_t *) SN_address, SERIAL_NUMBER_SIZE*2, aliaslist_mb ) ;
		// Add '='
		MemblobAdd( (uint8_t *) "=", 1, aliaslist_mb ) ;
		// Add alias name
		MemblobAdd( (const uint8_t *)CONST_TREE_DATA(p), p->dsize, aliaslist_mb ) ;
		// Add <CR>
		MemblobAdd( (uint8_t *) "\x0D\x0A", 2, aliaslist_mb ) ;
		return ;
	case preorder:
	case endorder:
		break;
...
void Cache_Add_Alias_Bus(const char * alias_name, int bus)
{
	// allocate space for the node and data
...
static void Cache_Add_Alias_Common(struct alias_tree_node *atn)
{
	...
	if ((opaque = tsearch(atn, &cache.temporary_alias_tree_new, alias_tree_compare))) {
...
/* Alias name must be a null-terminated string */
int Cache_Get_Alias_Bus(const char * alias_name)
{
	// allocate space for the node and data
	size_t datasize = strlen(alias_name) ;
	struct alias_tree_node *atn = (struct alias_tree_node *) owmalloc(sizeof(struct alias_tree_node) + datasize + 1);
...
/* Alias name must be a null-terminated string */
GOOD_OR_BAD Cache_Get_Alias_SN(const char * alias_name, uint8_t * sn )
{
	// allocate space for the node and data
	size_t datasize = strlen(alias_name) ;
...
/* Alias name must be a null-terminated string */
void Cache_Del_Alias_Bus(const char * alias_name)
{
	// Cheat -- just change to a bad bus value
	LEVEL_DEBUG("Hide %s",alias_name) ;
	Cache_Add_Alias_Bus( alias_name, -1 ) ;
}
